...
def send_totp(request):
	...
	try:
		...
		is_exist = session.query(UserTotp).filter(UserTotp.user_id == result.Users.id).filter(UserTotp.type == 0).first()
		totp = ''
		if not is_exist:
		...
		if int(request_body['auth_type']) == 1:
			flag = Helper.send_mail(result.Users.email,'OTP - LOGIN | Kore Task','totp',totp)
			if not flag:
...
def check_totp(request):
	try:
		...
		.filter(LinkHash.hash == request_body['token'])\
		.filter(UserTotp.type == 0)\
		.first()
	...
	try:
		...
		# For generating token .............................................
			...
			if result.is_active == 0:
				return Helper.construct_response(404, "User not yet verified", '')				
...
def change_password(request):
	try:
		...
		if existing_token.Users.is_active == 0:
			session.close()
